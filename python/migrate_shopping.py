...
class ShoppingMigrator:
    ...
    def extract_keywords_from_reviews(self, google_reviews: str) -> List[str]:
        ...
        try:
            ...
            # 쇼핑몰/시장 키워드
            shopping_keywords = re.findall(r'[가-힣]{2,4}(?:시장|쇼핑몰|백화점|아울렛|마트|편의점|상가|상점|가게|매장|점포|상점가|거리|상권|쇼핑센터|플라자)', all_text)
            
            # 쇼핑 품목 키워드
            item_keywords = re.findall(r'[가-힣]{2,4}(?:의류|옷|신발|가방|화장품|향수|액세서리|시계|귀걸이|목걸이|반지|팔찌|가죽|한복|의상|패션|뷰티|미용|헤어|네일|마사지|스파)', all_text)
            
            # 감정/평가 키워드
            emotion_keywords = re.findall(r'[가-힣]{2,4}(?:좋|나쁘|예쁘|멋있|훌륭|완벽|최고|추천|만족|불만족|감동|실망|재미있|즐겁|편리|편안|깔끔|아름다)', all_text)
            
            # 모든 키워드 합치기
            all_keywords = shopping_keywords + item_keywords + emotion_keywords
    ...
    def get_shopping_data(self) -> List[Dict[str, Any]]:
        """fetched_shopping 테이블에서 데이터 조회"""
        try:
            ...
            logger.info(f"fetched_shopping에서 {len(data)}건 조회")
            return data
    ...
    def transform_to_spots_data(self, shopping_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """fetched_shopping 데이터를 spots 형식으로 변환"""
        spots_data = []
    ...
    def migrate_data(self) -> bool:
        """데이터 마이그레이션 실행"""
        logger.info("쇼핑 데이터 마이그레이션 시작")
        
        try:
            # 1. fetched_shopping에서 데이터 조회
            shopping_data = self.get_shopping_data()
            if not shopping_data:
                logger.warning("마이그레이션할 데이터가 없습니다.")
                return False
            
            # 2. spots 형식으로 변환
            spots_data = self.transform_to_spots_data(shopping_data)
            ...
            if success:
                logger.info("✅ 쇼핑 데이터 마이그레이션 완료!")
            else:
                logger.error("❌ 쇼핑 데이터 마이그레이션 실패")
...
def main():
    ...
    # 마이그레이션 실행
    migrator = ShoppingMigrator(db_config)
